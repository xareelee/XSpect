...
IntrospectBlock Guard_UIShouldNotBeFirstResponder(UIView* aView){
    ...
    return ^ Matryoshka(Matryoshka innerBlock){
        return innerBlock;
    };
};
...
IntrospectBlock Guard_StringShouldContainCharacters(NSString *string, NSString *alertMessage){
    ...
    if (! string || [string length] == 0) {
        return ^ Matryoshka(Matryoshka innerBlock){
			return ^(){
				if (alertMessage) {
					UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:alertMessage delegate:nil cancelButtonTitle:@"確定" otherButtonTitles:nil];
					[alertView show];
				}
			};
		};

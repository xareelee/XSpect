...
/**
 ...
 Must Read –– About `AspectName` Macro Variable
 ==============================================
 
 `AspectName` is a macro variable which isn't defined in XAspect library, but is used for many macros in XAspect. You should define/redefine `AspectName` whenever you want to use XAspectExtension macros to define a category of the aspect.
 ...
 **Caution:** you should **ALWAYS REDEFINE** the keyword `AspectName` by using `#undef` and `#define` macros **JUST BEFORE** using any following macros. Those following macros will automatically use this keyword (`AspectName`, macro vairable) to resolve it's definition.
 
 To use the following macros, read the documentation attached to them.
 
 The following macros introduce and substitute the `AspectName` keyword for the name redefined by you. There are 3 levels of macros to accomplish this goal (interface macro -> prototype macro -> resolved macro):
 
 * interface macro: the macro for use. It will invoke the prototype macro and introduce the macro vairables (e.g. `AspectName`) which were defined in other place.
...
/** 
 ...
 Use the following macro structure to declare a category for aspect
     ...
     // Redefine the `AspectName` macro varaible
     #undef AspectName
     #define AspectName
 
     AspectClass(className) // Start to describe an aspect for the class
 ...
 */
#define AspectClass(className) PrototypeAspectCategory(className, AspectName)
...
/**
 `WeaveAspectClassMethods(classSelector, ...)` macro will swap multiple selectors of class methods for the class using it. It will use the `AspectName` as the prefix for the aspect methods (separated by a underscore).
 
 You should use this macro in the `+load` method or the configuration block in the `AspectClass(className) ... AspectImplementation ... EndAspect` format.
...
PrototypeWeaveAspectClassMethods(AspectName, [self class], (classSelector), ##__VA_ARGS__, nil)
...
/**
 ...
 `WeaveAspectInstanceMethods(instanceSelector, ...)` macro will swap multiple selectors of instance methods for the class using it. It will use the `AspectName` as the prefix for the aspect methods (separated by a underscore).
...
PrototypeWeaveAspectInstanceMethods(AspectName, [self class], (instanceSelector), ##__VA_ARGS__, nil)
...
/**
 This macro will concatenate into a prefixed method with a defined name (the keyword `AspectName` you defined). It will use the `AspectName` as the prefix for the aspect methods (separated by a underscore).
 
 This macro may not be useful because the automatic method completion won't be introduced.
 */
#define Aspect(method, ...) ProtypePrefixedMethod(AspectName, method, ##__VA_ARGS__)
